...
from utils.data import EstimatorDataset
import numpy as np
...
def run(config, trainloader, validatonloader, testloader, devloader=None):
    ...
    estimator = Estimator(config, checkpoint_directory)
    estimator.to(config['device']['device'])
    estimator.fit(trainloader, validatonloader)
    estimator.test(testloader)

    estimator.to('cpu')
    jit_example = torch.rand(1, config.getint("data", "seqeunce_length")*config.getint("data", "n_input_feature"))
    with torch.no_grad():
        traced_script_module = torch.jit.trace(estimator.forward, jit_example)
    traced_script_module.save("model.pt")

    for name, param in estimator.state_dict().items():
...
if __name__ == '__main__':
    ...
    if config.getboolean("log", "wandb") is True:
        wandb.init(project="Estimator", tensorboard=False)
    ...
    train_data = EstimatorDataset(train_csv_path,seq_len=data_seq_len, n_input_feat=data_num_input_feat, n_output=data_num_output)
    ...
    validation_data = EstimatorDataset(validation_csv_path,seq_len=data_seq_len, n_input_feat=data_num_input_feat, n_output=data_num_output)
    validationloader = DataLoader(validation_data,
                                batch_size=config.getint("training", "batch_size"),
                                shuffle=False,
                                drop_last=True,
                                num_workers=8,
    ...
    test_data = EstimatorDataset(test_csv_path,seq_len=data_seq_len, n_input_feat=data_num_input_feat, n_output=data_num_output)
